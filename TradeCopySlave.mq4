            ...
            if(OrdTyp[i] > 1 && OrdPrice[i] != RealOrdPrice[j])
              {
               if(OrderSelect(RealOrdId[j], SELECT_BY_TICKET))
                 {
                  bool a = OrderModify(OrderTicket(), OrdPrice[i], OrderStopLoss(), OrderTakeProfit(), 0);
                 }
              }
            //compare SL,TP
            if(IgnoreSLTP == false && (OrdTP[i] != RealOrdTP[j] || OrdSL[i] != RealOrdSL[j]))
              {
               if(OrderSelect(RealOrdId[j], SELECT_BY_TICKET))
                 {
                  bool b =  OrderModify(OrderTicket(), OrderOpenPrice(), OrdSL[i], OrdTP[i], 0);
                 }
              }
           }
        }
            ...
            if((OrdTyp[i] == OP_BUY  && Price < OrdPrice[i] + PipsTolerance * mp * Point) ||
               (OrdTyp[i] == OP_SELL && Price > OrdPrice[i] - PipsTolerance * mp * Point))
              {
               ...
               if(result > 0)
                 {
                  if(IgnoreSLTP == false)
                    { bool c = OrderModify(result, OrderOpenPrice(), OrdSL[i], OrdTP[i], 0);}
                 }
               else
      ...
      if(x[j] != 1)  //no master order, close the ticket
        {
         ...
         if(RealOrdTyp[j] < 2)
           ...
           {
            bool d = OrderDelete(RealOrdId[j], CLR_NONE);
           }
        }
     }
  }
